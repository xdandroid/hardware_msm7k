...
#undef CHECK_FOR
    ...
    if ( SND_DEVICE_IDLE != -1) {
        ...
        LOGE("Can't reset modem audio, SND_DEVICE_IDLE not defined");
    }
}
...
{
    LOGV("AudioHardware::setMicMute_nosync %d", state);
    if (mMicMute != state) {
        mMicMute = state;
...
status_t AudioHardware::doRouting()
{
    ...
    int (*msm72xx_enable_audpp)(uint16_t, uint32_t);
    msm72xx_enable_audpp = (int (*)(uint16_t, uint32_t))::dlsym(acoustic, "msm72xx_enable_audpp");
